...
# ==============================
# v4.3.3 FILLS AND LEVERAGE PATCH
# ==============================
...
def _parse_avg_price_and_qty_from_order_payload(payload):
    """
    BingX 주문/체결 응답 필드명이 버전별로 조금씩 달라서 가능한 키를 넓게 파싱한다.
    """
    ...
    if isinstance(obj, list):
        ...
        for item in obj:
            ...
            price = _safe_float(
                item.get("price") or item.get("filledPrice") or item.get("avgPrice") or item.get("dealPrice"),
                None
            )
            qty = _safe_float(
                item.get("qty") or item.get("quantity") or item.get("executedQty") or item.get("filledQty") or item.get("volume"),
                None
            )
...
async def set_bingx_leverage(api_key: str, api_secret: str, symbol: str, leverage: int = 4):
    """
    v4.3.3: 실전 공통 고정 레버리지 4배.
    BingX 계정/모드에 따라 endpoint가 지원되지 않을 수 있으므로,
    주문 전 시도하고 실패 시 오류를 호출부에서 처리한다.
    """
    symbol = normalize_bingx_symbol(symbol)
    params = {
        "symbol": symbol,
    ...
    return await _bingx_signed_request(
        api_key,
        api_secret,
        "POST",
...
async def get_bingx_order_detail(api_key: str, api_secret: str, symbol: str, order_id):
    """
    주문 상세 조회. 실패할 수 있어서 호출부에서 fallback 가능하게 둔다.
    """
    symbol = normalize_bingx_symbol(symbol)
    if not order_id:
        return None
...
async def get_bingx_order_fills(api_key: str, api_secret: str, symbol: str, order_id=None):
    """
    체결내역 조회. BingX 응답/엔드포인트 차이를 고려해서 여러 후보를 시도한다.
    """
    symbol = normalize_bingx_symbol(symbol)
    candidates = []
